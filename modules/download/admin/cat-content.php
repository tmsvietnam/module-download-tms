...
if ($nv_Request->isset_request('submit', 'post')) {
	...
	$check_return = $check_alias->check_catid_alias($catid, $array['alias']);
    ...
    if (empty($error)) {
        ...
        if (!empty($catid)) {
            ...
            if ($stmt->execute()) {
				...
				// Thêm id_alias
				$check_alias->add_alias_catalogy_news($catid, $array['alias'], $module_name, 'viewcat');
            ...
            if ($db->insert_id($sql, 'id', $data_insert)) {
				...
				// Thêm id_alias
				$check_alias->add_alias_catalogy_news($id_page, $array['alias'], $module_name, 'viewcat');
