...
if ($nv_Request->isset_request('submit', 'post')) {
	...
	$check_return = $check_alias->check_id_alias($id, $array['alias']);
    ...
    elseif (empty($array['title'])) {
        ...
        if (empty($error)) {
            ...
            if (empty($id) or $is_copy) {
				...
				// XỬ LÝ THÊM ALIAS
				// Thêm id_alias
				$check_alias->add_alias_news($array['catid'],$array['id'], $array['alias'], $module_name, 'viewfile');
                ...
                if (! $stmt->execute()) {
					...
					// XỬ LÝ THÊM ALIAS
					// Thêm id_alias
					$check_alias->add_alias_news($array['catid'],$id, $array['alias'], $module_name, 'viewfile');
			...
			if ($array['keywords'] != $array['keywords_old'] or $is_copy) {
				...
				$db->query('DELETE FROM ' . NV_MOD_TABLE . '_tags_id WHERE id = ' . $id_tam);
				
				// THÊM LẠI VÀO 
				if(!empty($array['keywords']))
				{
					$array_tag = explode(',',$array['keywords']);
					foreach($array_tag as $tag_new)
					{
						
						$db->query('INSERT INTO ' . NV_MOD_TABLE . '_tags_id (id, did, keyword) VALUES (' . $id_tam . ', ' . intval($tag_new) . ', ' . intval($tag_new) . ')');
					}
				}
			}
...
$list_tag_news = $db->query('SELECT * FROM ' . NV_MOD_TABLE . '_tags ORDER BY did ASC')->fetchAll();

// DANH SÁCH ALIAS CỦA ID NÀY

if($array['id'] > 0)
{
	$list_tag_id = $db->query('SELECT did FROM ' .  NV_MOD_TABLE . '_tags_id WHERE id ='. $array['id'])->fetchAll();
}

$array_tag_new = array();
if(!empty($list_tag_id))
{
	foreach($list_tag_id as $tag_id)
	{
		$array_tag_new[] = $tag_id['did'];
	}
}
//print_r($array);die;
foreach($list_tag_news as $tag)
{//print_r($array_tag_new);die;
	if(in_array($tag['did'],$array_tag_new))
	$xtpl->assign('selected_tag', 'selected=selected');
	else $xtpl->assign('selected_tag', '');
	$xtpl->assign('tag', $tag);
    $xtpl->parse('main.tag');
}
