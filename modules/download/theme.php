...
function view_file($row, $download_config, $content_comment, $array_keyword)
{
    ...
    if (!empty($array_keyword)) {
        ...
        foreach ($array_keyword as $i => $value) {
            $xtpl->assign('KEYWORD', $value['alias']);
            $xtpl->assign('LINK_KEYWORDS', nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='.$value['alias'],true));
